...
def decompose(r_stock, r_mkt, r_sec):
    ...
    return {
        ...
        "basis": (f"weekly total returns, {len(d)} weeks, regressed on {MARKET} "
                  ...
                  f"over this window, not a hedge ratio and not a forecast."),
    }
...
def main():
    ...
    secs = sorted(set(SECTOR_ETF.values()))
    ...
    for n in names:
        ...
        if r_y is None:
            ...
            skipped += 1
            continue
        etf = SECTOR_ETF.get(n.get("sector"))
        ...
        if dec.get("replicable_share") is not None:
            n["verdict_detail"] = (
                ...
                f"return variance over the past {dec['weeks'] // 52} years came from the "
